...
impl Db {
    pub async fn get_interface(&self, name: &str) -> Result<Option<(i64, u64, u64, Option<String>, Option<u8>, i64, i64, u64, u64)>> {
        let mut rows = self.local_conn.query(
            "SELECT id, rxcounter, txcounter, mac_address, interface_type, updated, created, rxtotal, txtotal FROM interface WHERE host_id = ? AND name = ?", 
        ...
        if let Some(row) = rows.next().await? {
            ...
            let itype: Option<u8> = row.get::<Option<i64>>(4)?.map(|v| v as u8);
            let updated: i64 = row.get(5)?;
            let created: i64 = row.get(6)?;
            let rxtotal: i64 = row.get(7)?;
            let txtotal: i64 = row.get(8)?;
    ...
    pub async fn create_interface(&self, name: &str, rx: u64, tx: u64, mac: Option<String>, itype: Option<u8>) -> Result<i64> {
        let now = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs() as i64;
        let local_sql = "INSERT OR IGNORE INTO interface (host_id, name, mac_address, interface_type, created, updated, rxcounter, txcounter) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        
        self.local_conn.execute(local_sql, (self.host_id, name.to_string(), mac.clone(), itype.clone(), now, now, rx as i64, tx as i64)).await?;
        let id = self.local_conn.last_insert_rowid();

        if let Some(ref remote) = self.remote_conn {
            let remote_sql = "INSERT OR IGNORE INTO interface (host_id, name, mac_address, interface_type, created, updated, rxcounter, txcounter) 
                              SELECT id, ?, ?, ?, ?, ?, ?, ? FROM host WHERE machine_id = ?";
            if let Err(e) = remote.execute(remote_sql, (name.to_string(), mac, itype, now, now, rx as i64, tx as i64, self.machine_id.clone())).await {
    ...
    pub async fn update_interface_counters(&self, id: i64, name: &str, rx: u64, tx: u64, rx_delta: u64, tx_delta: u64, rxtotal: u64, txtotal: u64, created: i64, mac: Option<String>, itype: Option<u8>) -> Result<()> {
        let now = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs() as i64;
        let local_sql = "UPDATE interface SET updated = ?, rxcounter = ?, txcounter = ?, rxtotal = rxtotal + ?, txtotal = txtotal + ?, interface_type = ? WHERE id = ?";
        
        self.local_conn.execute(local_sql, (now, rx as i64, tx as i64, rx_delta as i64, tx_delta as i64, itype.clone(), id)).await?;
        if let Some(ref remote) = self.remote_conn {
            let remote_sql = "INSERT INTO interface (host_id, name, mac_address, interface_type, created, updated, rxcounter, txcounter, rxtotal, txtotal)
                              SELECT id, ?, ?, ?, ?, ?, ?, ?, ?, ? FROM host WHERE machine_id = ?
            ...
            if let Err(e) = remote.execute(remote_sql, (
                name.to_string(), mac, itype, created, now, rx as i64, tx as i64, rxtotal as i64, txtotal as i64, self.machine_id.clone(),
    ...
    pub async fn remove_interface(&self, name: &str) -> Result<()> {
        if let Some((id, _, _, _, _, _, _, _, _)) = self.get_interface(name).await? {
    ...
    pub async fn rename_interface(&self, old_name: &str, new_name: &str) -> Result<()> {
        if let Some((id, _, _, _, _, _, _, _, _)) = self.get_interface(old_name).await? {
